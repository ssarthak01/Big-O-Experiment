   ...
   //ofstream.open("LinkedListTiming.csv");
    ...
    while(x < 40000) {
        ...
        for(int i = x; i < x+size; i++) {
            ...
            sumInsertCollisions += table->getNumOfInsertCollision();
        }
        ...
        for(int j = 0; j < size; j++) {
            ...
            //cout << randIndex << ", ";
            clock_t startTime = clock();
            table->searchItem(testData[randIndex]);
            clock_t endTime = clock();
            total100SeTime += (endTime-startTime);
            sumSearchCollisions += table->getNumOfSearchCollision();
        }
    ...
    outfile.open("HashChainingInsert.csv");
    for(int i = 0; i < 400; i++) {
        outfile << insertArr[i] << "\n";
    }
    outfile.close();

    outfile2.open("HashChainingSearch.csv");
    for(int i = 0; i < 400; i++) {
        outfile2 << searchArr[i] << "\n";
    }
    outfile2.close();

    outfile3.open("ChainingInsertCollisions.csv");
    for(int i = 0; i < 400; i++) {
        outfile3 << insertCollisionArr[i] << "\n";
    }
    outfile3.close();

    outfile4.open("ChainingSearchCollisions.csv");
    for(int i = 0; i < 400; i++) {
